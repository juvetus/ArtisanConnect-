...
type RequestItem = { id: string; category: string; city: string; neighborhood?: string | null; description: string; budgetMin?: number | null; budgetMax?: number | null; requestedDate?: string | null; fileUrls?: string[] | null; status: string; createdAt: string; deliveredAt?: string | null; paymentStatus?: 'unpaid' | 'pending' | 'paid'; paymentMethod?: 'momo' | 'cash' | null; paymentAmount?: number | null; client?: { name?: string | null; contactPreference?: 'platform' | 'whatsapp' | 'both'; contactPhone?: string | null }; matchScore?: number; targeted?: boolean; alreadyAnswered?: boolean; awarded?: boolean; awardedToMe?: boolean; myResponse?: MyResponse | null };
...
export default function ArtisanCustomerRequestsPage() {
  ...
  const suggestReply = async (request: RequestItem) => {
    setSuggestingId(request.id);
    setNotice('');
    try {
      const suggestion = await api.assistantSuggestOpportunityReply({
      ...
      const current = form[request.id] ?? { price: '', days: '', message: '' };
      setForm((previous) => ({ ...previous, [request.id]: { ...current, message: suggestion.content } }));
      setNotice('Brouillon IA ajouté au message. Vérifiez-le et complétez votre prix et délai avant envoi.');
    } catch (error) {
      setNotice(error instanceof Error ? error.message : 'Impossible de préparer une réponse.');
    } finally {
